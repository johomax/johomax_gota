...
IMPORTANT: your sandbox cannot run Docker, so you CANNOT run the benchmarks; the orchestrator runs them after you finish. Do not attempt
run-episode or tools/eval.py. Instead, self-check the BASIC carefully against the dialect rules (case-insensitive names, no elseif, etc.),
and describe exactly what to look for in the telemetry. For reference, the orchestrator will run:
...
Deliverable: write policy/<cand>.bas (one focused change on top of v5, keep everything else byte-identical where possible) and reply with:
the change (one paragraph), the exact code regions touched, any risks/assumptions, and what telemetry would confirm the mechanism.
Do not modify v5.bas, tools/, docs/, or git.
