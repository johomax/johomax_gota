' v367 = v363 + v364's hero hunting for carries (enemy heroes two or more levels below us within ten tiles score +300 when no enemy tower is within ten tiles)
...
sub readObject(index)
  ...
  elseif kind = 2 then
    ...
    ' v364: carries hunt weak heroes as the leaders do (150 XP + 100 gold each): an enemy hero two or more levels below us within ten tiles, no enemy tower within ten tiles of us
    if (selfClass = Ranger or selfClass = Crossbowman) and objectLevel(index) + 2 <= selfLevel and distance <= 100 and eTowerD > 100 then
      score = score + 300
    end if
  elseif kind = 3 then
    score = score + 100
