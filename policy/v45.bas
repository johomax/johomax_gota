...
' v45 = v30 adapted to game version 33 (towers 1200/2400/4800 HP, 28/56/112 dmg): siege only out of range or under footman cover, back out of tower reach otherwise.
...
siegeAllowed = 0
if towerId <> 0 and (rng > towerRange or waveAtTower >= 1 or fortId <> 0) then
  siegeAllowed = 1
end if
' inside a living tower's reach with no footman cover and no range advantage -> back out first
towerDanger = 0
if nearTowerRange > 0 and rng <= nearTowerRange and nearTowerWave = 0 and nearTowerD2 * 100 <= (nearTowerRange + 8) * (nearTowerRange + 8) then
  towerDanger = 1
end if
