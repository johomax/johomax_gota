  ...
  if t = selfTeam then
    ...
    if k = 3 then
      ...
      if objectAlive(i) = 1 and d2 <= farmR2 then
        footObjDx = x - ox
        footObjDy = y - oy
        footObjD2 = footObjDx * footObjDx + footObjDy * footObjDy
        if footObjD2 <= myD2 * 2 + 64 and hp < farmFootHp then
          farmFootHp = hp
          farmFoot = objectId(i)
        end if
      end if
...
if done = 0 and routing = 0 and farmFoot <> 0 and lowHp = 0 then
