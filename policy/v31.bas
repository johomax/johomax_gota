...
' Threat values come from the previous tick's scan.
if potSlot >= 0 then
  if (selfHp * 100 < selfMaxHp * 60 and (enemyHeroNear > 0 or towerId <> 0)) or selfHp * 100 < selfMaxHp * 45 then
    useItem(potSlot)
    potCount = potCount - 1
  end if
end if
...
' Refill first; the else chain permits at most one purchase per tick.
if potCount < 2 and selfGold >= 50 then
  buyItem(2)
  potCount = potCount + 1
else
  if bootsBought = 0 and selfGold >= 100 + shopReserve then
    buyItem(8)
    bootsBought = 1
  else
    if daggerBought = 0 and selfGold >= 110 + shopReserve then
      buyItem(11)
      daggerBought = 1
    else
      if axeBought = 0 and selfGold >= 180 + shopReserve then
        buyItem(18)
        axeBought = 1
      else
        if armorBought = 0 and selfGold >= 160 + shopReserve then
          buyItem(16)
          armorBought = 1
        end if
      end if
