' GOTA policy v7 "edge blitz, objective-first, no deadlocks": all five heroes push one side lane
...
while i < n
  ...
  if t = selfTeam then
    if k = 2 then
      if objectId(i) <> selfId and objectAlive(i) = 1 and d2 <= 625 and objectHp(i) > 60 then
        ex = x - ox
        ey = y - oy
        ad2 = ex * ex + ey * ey
...
if done = 0 and towerId <> 0 and lowHp = 0 and nearHeroD2 > 9 then
