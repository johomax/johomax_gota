' v375 = v363 + equipment is bought only when the gold left still covers the next buyback (scroll and potions exempt)
...
sub buy(id, price, quantity)
  ...
  ' v375: keep gold for the next buyback (100 per death so far plus one): the leaders buy back 3.3 times a game, we 1.2, and each respawn costs 14-60 seconds
  if id <> 21 and id <> 1 and budget - price < 100 * (selfDeaths + 1) then
    exit sub
  end if
