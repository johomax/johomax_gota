...
while i < n
  ...
  if t = selfTeam then
    ...
    if k = 4 and objectHp(i) > 0 and d2 <= 36 then
      ownTowerNear6 = 1
    end if
  else
    if k = 2 then
...
dy = selfY - homeY
homeD2 = dx * dx + dy * dy
dx = selfX - routeX(pushLane * 7 + 6)
dy = selfY - routeY(pushLane * 7 + 6)
enemyFortD2 = dx * dx + dy * dy
onOurHalf = 0
hpLimit = 50
if homeD2 < enemyFortD2 then
  onOurHalf = 1
  hpLimit = 25
end if
threat = 0
if enemyHeroNear > allyNear8 + 1 or (enemyHeroNear > 0 and selfHp * 100 < selfMaxHp * hpLimit) then
  threat = 1
end if
if routing = 0 and threat = 1 and fortId = 0 and ownTowerNear6 = 0 then
  threatUntil = worldTick + 48
end if
' Finish the retreat even after the threat clears or a friendly tower is near.
if routing = 0 and fortId = 0 and worldTick < threatUntil then
...
if done = 0 and routing = 0 and bestHero <> 0 and threat = 0 and (allyNear8 + 1 >= enemyHeroNear or bestHeroHp * 2 < selfHp) then
