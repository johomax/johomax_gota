...
' v34 = v30 + fight odds/retreat (v26) + wave-gated sieging (v28).
...
while i < n
  ...
  d2 = dx * dx + dy * dy
  if t = selfTeam then
    if k = 4 and objectHp(i) > 0 and d2 < ownTowerD2 then
      ownTowerD2 = d2
    end if
    if k = 3 and objectAlive(i) = 1 and towerId <> 0 then
      waveDx = x - towerX
      waveDy = y - towerY
...
siegeAllowed = 0
if towerId <> 0 and (waveAtTower >= 1 or towerHp <= 150 or (melee = 0 and rng > towerRange)) then
  siegeAllowed = 1
end if
...
if enemyHeroNear >= allyNear8 + 2 or (selfHp * 100 < selfMaxHp * 40 and nearHeroD2 <= 36) then
  retreatUntil = worldTick + 72
end if
if done = 0 and worldTick < retreatUntil and fortId = 0 and ownTowerD2 > 36 then
...
if done = 0 and routing = 0 and towerId <> 0 and lowHp = 0 and nearHeroD2 > 9 and siegeAllowed = 1 then
  attackTarget(towerId)
  done = 1
  act = 6
end if

if done = 0 and routing = 0 and bestHero <> 0 and (allyNear8 + 1 >= enemyHeroNear or bestHeroHp * 2 < selfHp) then
...
if done = 0 and routing = 0 and towerId <> 0 and lowHp = 0 and siegeAllowed = 1 then
  attackTarget(towerId)
  done = 1
  act = 6
end if

if done = 0 and routing = 0 and towerId <> 0 and lowHp = 0 and siegeAllowed = 0 and otype = 1 and oid = towerId then
  ' Wait behind the objective tower until a wave arrives.
  stepToward(towerX, towerY, rx, ry, 7)
  dx = sx - selfX
  dy = sy - selfY
