...
sub buyEquip(equipId, equipCost)
  if shopSlots < 5 and shopGold >= equipCost then
    bought = buyItem(equipId)
    shopGold = shopGold - bought * equipCost
    shopSlots = shopSlots + bought
  end if
end sub
...
while slot < 6
  ...
  if iid >= 5 then
    shopSlots = shopSlots + 1
  end if
  slot = slot + 1
wend
' Boots first; keep one inventory slot available for healing.
buyEquip(8, 100)
if selfHp * 100 < selfMaxHp * 50 and potSlot >= 0 then
  useItem(potSlot)
end if
if selfHp * 100 < selfMaxHp * 60 and potSlot < 0 then
  if shopGold >= 50 then
    bought = buyItem(2)
    shopGold = shopGold - bought * 50
  else
    if selfHp * 100 < selfMaxHp * 30 and shopGold >= 30 then
      bought = buyItem(1)
      shopGold = shopGold - bought * 30
    end if
  end if
end if
' Spend at actual prices; wounded melee can buy a permanent HP buffer.
buyEquip(7, 70)
buyEquip(11, 110)
if melee = 1 and selfHp * 100 < selfMaxHp * 60 then
  buyEquip(16, 160)
end if
buyEquip(18, 180)
buyEquip(19, 180)
