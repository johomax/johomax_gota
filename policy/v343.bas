' v343 = v342 + melee heroes camp too, from level 14 (richard's Demon Hunter averages 2244, ours 476)
' v342 = v339: melee heroes two levels up and healthy hunt enemy heroes (+200 priority) for the 150-XP kills
' v339 = v338 with tower sieging also limited to ranged heroes (the melee seat-3 numbers matched v330's sieging drag)
...
sub readObject(index)
  ...
  towerOk = 0
  if kind = 4 and laneIdx <= 2 and prevTanks >= 2 and prevEnemyPower = 0 and attackRange >= 4 then
    gdx = x - enemyX
    gdy = y - enemyY
  ...
  elseif kind = 2 then
    ...
    ' v342: melee duelists two levels up and above two-thirds health hunt heroes for the 150 XP; other non-carries still ignore healthy heroes beyond three tiles
    huntOk = 0
    if attackRange < 4 and selfLevel >= objectLevel(index) + 2 and selfHp * 3 >= selfMaxHp * 2 then
      huntOk = 1
    end if
    if selfClass <> Ranger and selfClass <> Crossbowman and distance > 9 and hp >= selfAttackDamage * 4 and huntOk = 0 then
      exit sub
    end if
...
camp = 0
if laneIdx <= 2 and prevCampHeroes = 0 and (selfLevel >= 12 and attackRange >= 4 or selfLevel >= 14) then
  camp = 1
end if
