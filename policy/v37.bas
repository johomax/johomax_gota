...
' Follow allied groups when idle between lane routes; preserve respawn rejoin.
if worldTick mod 120 = 0 and laneTravel = 0 and rejoining = 0 and laneChanged = 0 then
  routeLane = 0
  while routeLane < 3
    ...
    while groupAlly < liveAllies
      groupNear = 0
      routePoint = 2
      while routePoint <= 5 and groupNear = 0
        routeIndex = routeLane * 7 + routePoint
        routeDx = routeX(routeIndex) - allyX(groupAlly)
        routeDy = routeY(routeIndex) - allyY(groupAlly)
        routeD2 = routeDx * routeDx + routeDy * routeDy
        if routeD2 <= 225 then
          groupNear = 1
        end if
        routePoint = routePoint + 1
      wend
      laneAllies(routeLane) = laneAllies(routeLane) + groupNear
  ...
  routeLane = 0
  while routeLane < 3
    if laneAllies(routeLane) > laneAllies(clusterLane) or (laneAllies(routeLane) = laneAllies(clusterLane) and routeLane = pushLane and clusterLane <> 1) then
      clusterLane = routeLane
    end if
    routeLane = routeLane + 1
  wend
  if laneAllies(clusterLane) >= 2 and clusterLane <> pushLane and worldTick - lastLaneTick >= 600 and enemyHeroNear = 0 and towerId = 0 then
    adoptLane(clusterLane, 2)
    laneTravel = 1
    dx = homeX - selfX
    dy = homeY - selfY
    groupHomeD2 = dx * dx + dy * dy
    dx = wpX(6) - selfX
    dy = wpY(6) - selfY
    if groupHomeD2 <= dx * dx + dy * dy then
      nearestRouteD2 = 1000000
      routePoint = 0
      while routePoint <= firstOi
        routeIndex = pushLane * 7 + routePoint
        routeDx = routeX(routeIndex) - selfX
        routeDy = routeY(routeIndex) - selfY
        routeD2 = routeDx * routeDx + routeDy * routeDy
        if routeD2 < nearestRouteD2 then
          nearestRouteD2 = routeD2
          oi = routePoint
        end if
        routePoint = routePoint + 1
    ...
    print "GROUP " ; worldTick ; " lane " ; clusterLane ; " allies " ; laneAllies(clusterLane)
  end if
end if
...
if routing = 0 and selfHp * 100 < selfMaxHp * 15 and enemyHeroNear > 0 then
  walkTo(rx, ry)
  done = 1
  act = 1
end if
