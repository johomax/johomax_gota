...
while i < n
  ...
  d2 = dx * dx + dy * dy
  if t = selfTeam then
    ' Towers precede footmen, so both tower positions are final here.
    if melee = 0 and k = 3 and objectAlive(i) = 1 then
      if towerId <> 0 then
        waveDx = x - towerX
        waveDy = y - towerY
    ...
    if k = 4 then
      ...
      ' Unexposed towers still shoot.
      if melee = 0 and objectHp(i) > 0 and d2 < nearTowerD2 then
        nearTowerD2 = d2
        nearTowerX = x
        nearTowerY = y
...
if melee = 0 then
  towerAllowed = 0
  if towerId <> 0 then
    if rng > towerRange or waveAtTower >= 1 or towerHp <= 150 or fortId <> 0 then
      towerAllowed = 1
    end if
  end if
end if
...
if done = 0 and routing = 0 and towerId <> 0 and lowHp = 0 and nearHeroD2 > 9 and towerAllowed = 1 then
...
if done = 0 and routing = 0 and towerId <> 0 and lowHp = 0 and towerAllowed = 1 then
  attackTarget(towerId)
  done = 1
  act = 6
end if

if done = 0 and melee = 0 and towerAllowed = 0 and nearTowerWave = 0 and nearTowerD2 * 100 <= (nearTowerRange + 5) * (nearTowerRange + 5) then
...
if done = 0 and lowHp = 1 and routing = 0 then
  ...
  if selfHp * 100 < selfMaxHp * 20 and selfGold < 30 and potSlot < 0 and towerId <> 0 and enemyHeroNear = 0 and towerAllowed = 1 then
    attackTarget(towerId)
    done = 1
    act = 12
  end if
