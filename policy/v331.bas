' v331 = v329 with farmers casting down to a quarter of their mana (was half): faster wave clears
' v329 = v317 with a farmer shop list: scroll, potion, boots, Rune Crossbow, Battle Axe, Knight Armor, potions (damage and HP instead of two scroll stacks and mana potions)
...
sub inventory()
  ...
  budget = selfGold
  ' v329: a farmer's six slots: scroll first from the keep (one), then boots, Rune Crossbow (+14), Battle Axe (+14), Knight Armor (+120 HP), one potion stack.
  ' richard v153's Ranger carries boots, bow, crossbow, buckler and amulet; the baseline list spent the slots on two scroll stacks and mana potions.
  if laneIdx <= 2 then
    buy(21, 100, 1)
    buy(1, 30, 1)
    buy(8, 100, 1)
...
sub spells()
  for spellSlot = 0 to 3
    ...
    if abilityLevel(spellSlot) > 0 and charges > 0 then
      if abilityCooldown(spellSlot) = 0 and selfMana >= cost and (laneIdx >= 3 or selfMana * 4 >= selfMaxMana) then
        castId = 0
