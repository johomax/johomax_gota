' v389 = v385 + ranged heroes step three tiles away from a melee bruiser (level at least ours minus one) that targets them from within 3.5 tiles
...
sub readObject(index)
  ...
  ' v389: a melee bruiser (VK, Druid, DH, DK, Berserker) that targets a ranged hero of ours from within 3.5 tiles gets kited: richard's VK aimed 3000+ attacks at one Crossbowman
  if kind = 2 and distance <= 12 and objectTarget(index) = selfId and objectLevel(index) + 1 >= selfLevel then
    bruiserClass = objectClass(index)
    if bruiserClass = 0 or bruiserClass = 3 or bruiserClass = 4 or bruiserClass = 5 or bruiserClass = 9 then
      kiteFromX = x
      kiteFromY = y
      kiteUntil = worldTick + 30
    end if
  end if
...
if worldTick < kiteUntil and attackRange >= 4 and selfRootTicks = 0 then
  kiteX = selfX
  kiteY = selfY
  if kiteFromX >= selfX then
    kiteX = selfX - 3
  else
    kiteX = selfX + 3
  end if
  if kiteFromY >= selfY then
    kiteY = selfY - 3
  else
    kiteY = selfY + 3
  end if
  if kiteX < 1 then
    kiteX = 1
  elseif kiteX > mapWidth - 2 then
    kiteX = mapWidth - 2
  end if
  if kiteY < 1 then
    kiteY = 1
  elseif kiteY > mapHeight - 2 then
    kiteY = mapHeight - 2
  end if
