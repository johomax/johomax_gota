' v297 = v296: farmers never walk home for health (potions, buyback at 100 x deaths, portal scroll back); they step to the lane tower only below half health when outnumbered
...
if selfHp * 4 < selfMaxHp and laneIdx >= 3 then
  retreating = 1
end if
...
if enemyPower > friendlyPower + 6 and threatDistance < 64 then
  if selfHp * 4 < selfMaxHp * 3 and (laneIdx >= 3 or selfHp * 2 < selfMaxHp) then
  fleeX = homeX
  fleeY = homeY
  if laneIdx <= 2 and forwardDistance < 1000000 then
    fleeX = forwardX
...
elseif laneIdx >= 3 then
  ...
  gSeen = 0
  if laneIdx = 3 and f0Seen = 1 then
    gSeen = 1
    gX = f0X
    gY = f0Y
  end if
  if laneIdx = 4 and f1Seen = 1 then
    gSeen = 1
    gX = f1X
    gY = f1Y
  end if
...
if worldTick mod 240 < 6 then
  print "F " ; worldTick ; " lane " ; laneIdx ; " at " ; selfX ; " " ; selfY ; " L" ; selfLevel ; " goal " ; goalX ; " " ; goalY ; " tanks " ; tanks ; " ecreeps6 " ; nearCreeps ; " hp " ; selfHp ; " deaths " ; selfDeaths
end if
