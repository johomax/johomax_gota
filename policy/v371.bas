' v371 = v370 + health potions drunk whenever 120 HP are missing, threats or not (recovery is not interrupted by damage)
' v370 = v369 + a lane switch portals to the new lane's tower (scroll used from the field; walks of 100-130 tiles seen after FLOAT switches)
' v369 = v363 with three health potions bought right after the scroll and retreats cancelled once potions restore three fifths health
    ...
    if id = 0 then
      ...
      if itemCooldown(itemSlot) = 0 and inOwnSpawn() = 0 then
        ...
        if id = 1 and selfMaxHp - selfHp >= 120 then
          ' v371: potion recovery is never interrupted by damage (engine recoverPotion), so drink at the front too; relh's heroes use consumables 80 times in 8000 ticks, ours 4
          consume = 1
...
' v369: a retreat ends as soon as potions bring us back to three fifths health (120 HP over ten seconds each); no more 1500-tick walks home at level 2
if retreating and selfHp * 5 >= selfMaxHp * 3 and inOwnSpawn() = 0 then
  retreating = 0
end if
...
' v370: after a lane switch the scroll works from the field too: portal to the new lane's tower instead of walking up to 130 tiles
if laneIdx <= 2 and (canShop() = 1 or switchPortal = 1) and owned(21) > 0 and selfPortalCooldown = 0 and forwardDistance < 1000000 and inventorySlot(21) >= 0 then
  pdx = selfX - forwardX
  pdy = selfY - forwardY
  if pdx * pdx + pdy * pdy > 400 then
