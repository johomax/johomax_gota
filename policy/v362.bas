' v362 = v348 with the fallback goals fixed (lane 1 = centre, lane 2 = far corner) and the lane chosen every 600 ticks by the nearest enemy wave (the enemy creep closest to our god per lane, plus an ally penalty) instead of the seat lane + FLOAT rule
...
sub readObject(index)
  ...
  ' v362: per lane, remember the enemy creep nearest our god (how deep the enemy wave stands on our side)
  if kind = 3 then
    hdx = x - homeX
    hdy = y - homeY
    hd = hdx * hdx + hdy * hdy
...
camp = 0
if laneIdx <= 2 and selfLevel >= 12 and prevCampHeroes = 0 and attackRange >= 4 and selfDeaths < 5 then
  camp = 1
end if
' v362: every 600 ticks farm the lane whose enemy wave stands nearest our god (short trips, towers beside us, nobody to share
' with); cost = squared distance of that wave from our god + squared distance from us + 6000 per allied hero in the lane region; the current lane keeps a 3000 bonus
if worldTick >= 600 and worldTick - lastLaneChange >= 1200 and worldTick mod 600 < 6 and camp = 0 then
  bestCost = 1000000
  newLane = laneIdx
  for wl = 0 to 2
