' v394 = v385 + no FLOAT lane moves (v388) + fight back at attackers within three tiles (v392): the two derivatives that read best against the lane-0 control
...
sub readObject(index)
  ...
  elseif kind = 2 then
    ...
    ' v392: fight back — an enemy hero attacking us from within three tiles outranks any creep
    if objectTarget(index) = selfId and distance <= 9 then
      score = score + 400
    end if
  elseif kind = 3 then
    score = score + 100
...
' v388: no lane moves
if floater = 2 and worldTick >= 1200 and worldTick - lastLaneChange >= 2400 and worldTick mod 600 < 6 then
  curAllies = allyLane0
  if laneIdx = 1 then
    curAllies = allyLane1
