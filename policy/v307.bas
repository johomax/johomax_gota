' v307 = v306: lane goals match the lane regions (farmer 1 = mid, farmer 2 = top-right L; they were swapped since v292), and near enemy towers farmers only last-hit so our creeps do not kill the enemy god early
    ...
    if heroAvailable(candidate) then
      ...
      for player = 0 to draftPlayerCount() - 1
        if draftPlayerTeam(player) = selfTeam then
          ...
          if picked >= 0 then
            if heroRole(picked) = role and role <> 1 and role <> 2 then
              score = score - 100
            end if
            if heroRole(picked) = role and (role = 1 or role = 2) then
              score = score - 15
            end if
          end if
        end if
      ...
      if candidate = Ranger or candidate = Berserker then
        score = score + 2
      end if
...
sub readObject(index)
  ...
  ' v307: inside 14 tiles of an enemy tower a farmer only last-hits, so our wave does not push through their base and end the game
  if kind = 3 and laneIdx <= 2 and prevETowerD <= 196 and hp > selfAttackDamage then
    exit sub
  end if
  if kind = 1 then
    enemyX = x
  ...
  ' v292: buildings and the god are never targets (ending the game stops the farm)
  if kind <> 2 and kind <> 3 then
    exit sub
  end if
...
sub inventory()
  ...
  budget = selfGold
  ' v299: farmers buy a portal scroll before boots and teleport to their lane tower (the walk out costs ~0.5 min and the first waves)
  if laneIdx <= 2 then
    buy(21, 100, 1)
  end if
  buy(8, 100, 1)
...
' v306: three or more enemy heroes within 22 tiles of our god means a push that would end the game (relh/richard finish by minute 8-11):
' farmers come home, by scroll when they hold one, fight under the towers, and go back to their lanes when the threat is gone
if laneIdx <= 2 and baseThreat >= 3 then
  hdx = selfX - homeX
  hdy = selfY - homeY
  if hdx * hdx + hdy * hdy > 900 and owned(21) > 0 and selfPortalCooldown = 0 and inventorySlot(21) >= 0 and threatDistance > 100 then
    accepted = useItemAt(inventorySlot(21), homeX, homeY)
    if accepted then
      print "DEFEND-PORTAL " ; worldTick ; " threat " ; baseThreat
      end
    end if
  end if
  if worldTick mod 240 < 6 then
    print "DEFEND " ; worldTick ; " threat " ; baseThreat
  end if
  moveTo(homeX, homeY, 1)
  end
end if
