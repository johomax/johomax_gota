' v384 = v363 + melee heroes follow the strongest allied hero within 60 tiles (level at least theirs) instead of holding a lane front alone
...
sub readObject(index)
  ...
  if team = selfTeam then
    ...
    elseif kind = 2 then
      ...
      allies = allies + 1
      if id <> selfId then
        ' v384: remember the strongest allied hero in reach; melee farmers stand beside it instead of dying alone at a lane front
        if objectLevel(index) > buddyLevel or (objectLevel(index) = buddyLevel and distance < buddyD) then
          buddyLevel = objectLevel(index)
          buddyD = distance
          buddyX = x
          buddyY = y
        end if
...
' v384: a melee hero (range under four tiles) follows the strongest allied hero within 60 tiles when that ally is at least its level: shared creep XP beside a winner beats a solo lane death (melee seats average ~100 with 78% zeros)
if attackRange < 4 and camp = 0 and buddyLevel >= selfLevel and buddyD <= 3600 then
  goalX = buddyX
  goalY = buddyY
  if homeX > buddyX then
    goalX = buddyX + 2
  else
    goalX = buddyX - 2
  end if
  if homeY > buddyY then
    goalY = buddyY + 2
  else
    goalY = buddyY - 2
  end if
