' v397 = v385 with the lane move driven by crowd cost (allied heroes + 2 x enemy heroes at least our level per region): leave a lane costing 2+ for an empty one, every 2400 ticks, max three moves
...
sub readObject(index)
  ...
  ' v397: enemy heroes at least our level, per lane region — a lane full of them is a lane to leave
  if kind = 2 and objectLevel(index) >= selfLevel then
    if x <= 13 or y <= 17 then
      enemyLane0 = enemyLane0 + 1
    end if
    if x >= 102 or y >= 98 then
      enemyLane2 = enemyLane2 + 1
    end if
    laneS = x + y - 115
    if laneS < 0 then
      laneS = 0 - laneS
...
if floater = 1 and worldTick >= 1200 and worldTick - lastLaneChange >= 2400 and worldTick mod 600 < 6 then
  curAllies = allyLane0 + 2 * enemyLane0
  if laneIdx = 1 then
    curAllies = allyLane1 + 2 * enemyLane1
  elseif laneIdx = 2 then
    curAllies = allyLane2 + 2 * enemyLane2
  end if
  newLane = laneIdx
  bestAllies = curAllies
  if allyLane2 + 2 * enemyLane2 < bestAllies then
    bestAllies = allyLane2 + 2 * enemyLane2
    newLane = 2
  end if
  if allyLane0 + 2 * enemyLane0 < bestAllies then
    bestAllies = allyLane0 + 2 * enemyLane0
    newLane = 0
  end if
  if allyLane1 + 2 * enemyLane1 < bestAllies then
    bestAllies = allyLane1 + 2 * enemyLane1
    newLane = 1
  end if
  ' v397: crowd cost = allied heroes + twice the enemy heroes at least our level in the region; leave a lane costing 2+ for a lane costing 0
  if newLane <> laneIdx and curAllies >= 2 and bestAllies = 0 and switches < 3 then
    laneIdx = newLane
    lastLaneChange = worldTick
    switches = switches + 1
    print "FLOAT " ; worldTick ; " lane " ; laneIdx ; " allies " ; allyLane0 ; " " ; allyLane1 ; " " ; allyLane2 ; " enemies " ; enemyLane0 ; " " ; enemyLane1 ; " " ; enemyLane2
  end if
end if
