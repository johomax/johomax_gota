' v395 = v385 + melee heroes camp the enemy base from level 10 (same guards: no enemy hero near the camp point, fewer than four deaths)
...
if laneIdx <= 2 and prevCampHeroes = 0 and selfDeaths < 4 then
  ...
  ' v395: melee heroes camp from level 10 as well (lane 2 gets them there now; the camp is worth ~200 for ranged heroes)
  if selfLevel >= 10 and attackRange < 4 then
    camp = 1
  end if
end if
