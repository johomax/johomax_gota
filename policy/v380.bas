' v380 = v363 + shopping trips home for the crossbow and then the axe once 320 gold is banked (every 4800 ticks at most), crossbow ahead of boots in the shop list
...
' v380: shopping trip — creeps have 60 HP, so 60 damage makes every hit a last hit (15 gold and the 15% XP bonus); the leaders carry two
' weapons by tick 4800 while our gold sat idle (1560 at tick 14400): walk home for the crossbow, then the axe, as soon as the gold is there
wantItem = 0
if owned(19) = 0 then
  wantItem = 19
elseif owned(18) = 0 then
  wantItem = 18
end if
if wantItem > 0 and selfGold >= 320 and camp = 0 and canShop() = 0 and worldTick >= 1200 and worldTick - lastShopTrip >= 4800 then
  shopping = 1
end if
if shopping = 1 then
  if canShop() = 1 then
    shopping = 0
    lastShopTrip = worldTick
  else
    moveTo(spawnX, spawnY, 0)
    end
  end if
