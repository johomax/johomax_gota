' v321 = v317 with lane switches only to an EMPTY region within 50 tiles when two or more allies crowd ours
...
if floater = 1 and worldTick >= 1200 and worldTick - lastLaneChange >= 2400 and worldTick mod 600 < 6 then
  ...
  newLane = laneIdx
  bestD = 2500
  ' v321: only an EMPTY region counts, and only within 50 tiles of us (long walks cost more than a shared lane)
  cdx = selfX - mapWidth \ 10
  cdy = selfY - mapHeight \ 10
  if allyLane0 = 0 and laneIdx <> 0 and cdx * cdx + cdy * cdy < bestD then
    bestD = cdx * cdx + cdy * cdy
    newLane = 0
  end if
  cdx = selfX - mapWidth \ 2
  cdy = selfY - mapHeight \ 2
  if allyLane1 = 0 and laneIdx <> 1 and cdx * cdx + cdy * cdy < bestD then
    bestD = cdx * cdx + cdy * cdy
    newLane = 1
  end if
  cdx = selfX - mapWidth * 9 \ 10
  cdy = selfY - mapHeight * 9 \ 10
  if allyLane2 = 0 and laneIdx <> 2 and cdx * cdx + cdy * cdy < bestD then
    bestD = cdx * cdx + cdy * cdy
    newLane = 2
  end if
