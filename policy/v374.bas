' v374 = v363 + carries flee ten seconds from a hunter (two levels above, within twelve tiles) that targets them + heroes with more than six tiles of range close to 5.5 tiles before shooting creeps (creep XP reaches six tiles)
...
sub readObject(index)
  ...
  ' v374: carries flee too, but only when such a hunter is actually attacking us
  if kind = 2 and distance <= 144 and objectLevel(index) >= selfLevel + 2 and objectTarget(index) = selfId then
    hunterTargetUntil = worldTick + 240
  end if
...
if worldTick < hunterTargetUntil and (selfClass = Ranger or selfClass = Crossbowman) then
  fleeNow = 1
end if
...
if bestId <> 0 then
  ...
  ' v374: a creep that dies beyond six tiles gives no XP (CreepXpRange = 6 tiles; Crossbowman range 6.5): close to five and a half tiles first
  if bestKind = 3 and attackRange > 6 and bestDistance > 30 then
    moveTo(bestX, bestY, 0)
    end
  end if
