' v404 = v403 with the camp point at a twentieth of the way (~2.5 tiles from the enemy god) to bracket the optimum
' v403 = v385 with the camp point moved from a ninth to a fourteenth of the way from the enemy god to the centre
...
sub observe()
  ...
  prevFrontY = frontY
  frontAllies = 0
  ' v403: camp point a fourteenth of the way to the centre (~3.5 tiles from the enemy god) instead of a ninth (~5.5): the leaders hug the god where all three lanes' creeps spawn
  campX = enemyX + (mapWidth \ 2 - enemyX) \ 20
  campY = enemyY + (mapHeight \ 2 - enemyY) \ 20
  bestScore = -10000
  frontD = 1000000
  nearCreeps = 0
  baseThreat = 0
