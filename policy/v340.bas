' v340 = v339: campers enter the base only with an allied wave (or when already inside) and drink potions below 60% regardless of threats
    ...
    if id = 0 then
      ...
      if itemCooldown(itemSlot) = 0 and inOwnSpawn() = 0 then
        ...
        if id = 1 and selfMaxHp - selfHp >= 60 then
          ...
          if camp = 1 and selfHp * 5 < selfMaxHp * 3 then
            consume = 1
          end if
...
if laneIdx <= 2 and selfLevel >= 12 and prevCampHeroes = 0 and attackRange >= 4 then
  cdx = selfX - campX
  cdy = selfY - campY
  if prevTanks >= 1 or cdx * cdx + cdy * cdy <= 144 then
    camp = 1
  end if
end if
