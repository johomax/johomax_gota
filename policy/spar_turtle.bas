' SPARRING PARTNER turtle: v5 targeting, but holds at its own gate tower of the lane the enemy pushes: all five heroes push one side lane
...
if inited = 0 then
  ...
  nObj = 7
  oi = 0
  ' TURTLE sparring partner: hold at the mirror lane's own gate tower and defend
  gateId = 10 + (2 - pushLane) * 6 + selfTeam * 3 + 2
  i = 0
  n = objectCount()
  while i < n
    if objectTeam(i) = selfTeam and objectKind(i) = 4 and objectId(i) = gateId then
      gx = objectX(i)
      gy = objectY(i)
    end if
    i = i + 1
