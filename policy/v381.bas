' v381 = v363 + spells: free abilities and restores fire on cooldown, damage spells always fire for a creep last hit, the half-mana gate only holds back other damage casts
...
sub spells()
  for spellSlot = 0 to 3
    ...
    if abilityLevel(spellSlot) > 0 and charges > 0 then
      ' v381: the half-mana gate only holds back damage spells that are not last hits; free (no-mana) abilities and restores always fire, and a spell
      ' that finishes a creep is a last hit (15 gold, 15% XP) worth its mana — the leaders cast 1000-3000 times a game, we 14-34
      if abilityCooldown(spellSlot) = 0 and selfMana >= cost then
        castId = 0
        gateOk = 0
        if laneIdx >= 3 or selfMana * 2 >= selfMaxMana or cost = 0 or restore > 0 or healing > 0 then
          gateOk = 1
        end if
        if damage > 0 and bestId <> 0 and bestKind = 3 and bestHp <= damage then
          gateOk = 1
        end if
        if gateOk = 0 then
          damage = 0
