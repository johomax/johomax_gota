...
' v30 = v29 without the ally hold (never wait for strangers).
' v41 = v30 with temporary defense of threatened friendly towers.
...
dy = oy - selfY
myD2 = dx * dx + dy * dy
if defendMode = 0 and otype = 0 and myD2 <= 25 and oi < nObj - 1 then
  oi = oi + 1
  stuckHits = 0
  ox = wpX(oi)
  oy = wpY(oi)
...
while i < n
  ...
  d2 = dx * dx + dy * dy
  if t = selfTeam then
    if k = 4 and objectHp(i) > 0 and ownTowerCount < 9 then
      ownTX(ownTowerCount) = x
      ownTY(ownTowerCount) = y
      ownTId(ownTowerCount) = objectId(i)
      ownTowerCount = ownTowerCount + 1
    end if
    if k = 2 then
      ' Count every living friendly hero, including self, for defense support.
      if objectAlive(i) = 1 and seenAllies < 10 then
        allyX(seenAllies) = x
        allyY(seenAllies) = y
        seenAllies = seenAllies + 1
      end if
...
threatD2 = 1000000
threatIndex = 0
while threatIndex < ownTowerCount
  towerThreats = 0
  threatEnemyIndex = 0
  while threatEnemyIndex < seenEnemies
    threatDx = seenEnemyX(threatEnemyIndex) - ownTX(threatIndex)
    threatDy = seenEnemyY(threatEnemyIndex) - ownTY(threatIndex)
    if threatDx * threatDx + threatDy * threatDy <= 144 then
      towerThreats = towerThreats + 1
    end if
    threatEnemyIndex = threatEnemyIndex + 1
  wend
  if towerThreats > threatCount then
    threatTower = threatIndex
    threatTowerId = ownTId(threatIndex)
    threatCount = towerThreats
  end if
  threatIndex = threatIndex + 1
wend
if threatTower >= 0 then
  threatAllyIndex = 0
  while threatAllyIndex < seenAllies
    threatDx = allyX(threatAllyIndex) - ownTX(threatTower)
    threatDy = allyY(threatAllyIndex) - ownTY(threatTower)
    if threatDx * threatDx + threatDy * threatDy <= 144 then
      threatAllies = threatAllies + 1
    end if
    threatAllyIndex = threatAllyIndex + 1
  wend
  threatDx = ownTX(threatTower) - selfX
  threatDy = ownTY(threatTower) - selfY
  threatD2 = threatDx * threatDx + threatDy * threatDy
end if
if defendMode = 1 then
  if threatCount = 0 then
    defendQuietTicks = defendQuietTicks + 1
  else
    defendQuietTicks = 0
  end if
  if defendQuietTicks >= 240 or fortId <> 0 then
    defendMode = 0
    print "DEFEND end" ; worldTick
  end if
end if
if defendMode = 0 and threatCount >= 2 and threatCount >= threatAllies and threatD2 <= 3600 and fortId = 0 then
  defendMode = 1
  defendQuietTicks = 0
  print "DEFEND " ; worldTick ; " tower " ; threatTowerId ; " enemies " ; threatCount
end if
if defendMode = 1 and threatCount > 0 then
  stepToward(ownTX(threatTower), ownTY(threatTower), homeX, homeY, 2)
  defendX = sx
  defendY = sy
end if

if defendMode = 0 and objDead = 1 and otype = 1 and oi < nObj - 1 then
  oi = oi + 1
  stuckHits = 0
end if
...
rejoinStarted = 0
if liveAllies = 0 or defendMode = 1 then
  rejoining = 0
else
...
if defendMode = 0 and rejoining = 0 and laneChanged = 0 and resistance >= 72 and worldTick - lastLaneTick >= 1500 and otype = 1 and objStanding = 1 and objDead = 0 then
  switchLane = pushLane
  safestD2 = -1
  routeLane = 0
  while routeLane < 3
...
routing = 0
if defendMode = 0 and (laneTravel = 1 or rejoining = 1 or laneChanged = 1) then
  ox = wpX(oi)
  oy = wpY(oi)
  oid = wpId(oi)
  otype = wpKind(oi)
...
' Keep the lane waypoint intact; retain the last defense point during quiet ticks.
if defendMode = 1 then
  ox = defendX
  oy = defendY
  dx = ox - selfX
