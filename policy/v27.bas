...
' Prefer lanes with fewer towers and defenders, checking every 240 ticks.
if worldTick >= 720 and worldTick mod 240 = 0 and enemyHeroNear = 0 and selfHp * 100 >= selfMaxHp * 30 then
  bestLane = pushLane
  bestLaneScore = 1000000
  currentLaneEnemies = 0
  routeLane = 0
  while routeLane < 3
    laneScore = 0
    routePoint = 3
    while routePoint < 6
      if routeDead(routeLane * 7 + routePoint) = 0 then
        laneScore = laneScore + 10
      end if
      routePoint = routePoint + 1
    wend
    ...
    while enemyIndex < seenEnemies
      routeDx = seenEnemyX(enemyIndex) - routeX(routeIndex)
      routeDy = seenEnemyY(enemyIndex) - routeY(routeIndex)
      routeD2 = routeDx * routeDx + routeDy * routeDy
      if routeD2 <= 144 then
        laneEnemies = laneEnemies + 1
      end if
      enemyIndex = enemyIndex + 1
    wend
    laneAllies = 0
    enemyIndex = 0
    while enemyIndex < seenAllies
      routeDx = allyX(enemyIndex) - routeX(routeIndex)
      routeDy = allyY(enemyIndex) - routeY(routeIndex)
      routeD2 = routeDx * routeDx + routeDy * routeDy
      if routeD2 <= 144 then
        laneAllies = laneAllies + 1
      end if
      enemyIndex = enemyIndex + 1
    wend
    laneScore = laneScore + 4 * laneEnemies - 3 * laneAllies
    if routeLane = pushLane then
      laneScore = laneScore - 2
      currentLaneEnemies = laneEnemies
    end if
    if laneScore < bestLaneScore or (laneScore = bestLaneScore and routeLane = pushLane) then
      bestLaneScore = laneScore
      bestLane = routeLane
    end if
    routeLane = routeLane + 1
  wend
  if bestLane <> pushLane and worldTick - lastLaneTick >= 1200 and laneChanged = 0 then
    if oi < 5 or currentLaneEnemies >= 3 then
      adoptLane(bestLane, 1)
      laneTravel = 1
    end if
  end if
end if
