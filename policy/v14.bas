...
' ---------- fight area: allies within 8 tiles (including me) ----------
fightAllies = 0
i = 0
n = objectCount()
while i < n
  if objectKind(i) = 2 and objectTeam(i) = selfTeam and objectAlive(i) = 1 then
    dx = objectX(i) - selfX
    dy = objectY(i) - selfY
    if dx * dx + dy * dy <= 64 and fightAllies < 5 then
    ...
    if k = 2 then
      ...
      if objectAlive(i) = 1 then
        ...
        while j < fightAllies
          ex = x - fightAllyX(j)
          ey = y - fightAllyY(j)
          if ex * ex + ey * ey <= 144 then
            inFight = 1
          end if
          j = j + 1
        wend
        if inFight = 1 then
          if hp < bestHeroHp or (hp = bestHeroHp and objectId(i) < bestHero) then
            bestHeroHp = hp
            bestHero = objectId(i)
            bestHeroD2 = d2
          end if
...
if rejoining = 0 and laneChanged = 0 and resistance >= 72 and worldTick - lastLaneTick >= 1500 and otype = 1 and objStanding = 1 and objDead = 0 and oi = 3 then
  switchLane = pushLane
  safestD2 = -1
  routeLane = 0
  while routeLane < 3
...
if done = 0 and routing = 0 and melee = 0 and nearMeleeD2 <= 4 and (selfHp * 100 < selfMaxHp * 70 or nearMeleeHp > selfHp) then
