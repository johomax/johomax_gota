' v355 = v354 + brawl mode: before tick 1800 enemy heroes get +150 target priority while we are above half health
' v354 = v352 with outer/inner towers siegeable whenever two allied creeps are near, enemies present or not (relh takes the mid outer tower at tick 1200 in a crowd)
' v352 = v338 + opening brawl at the map centre until tick 1800 (no opening portal); then the usual lane
...
sub readObject(index)
  ...
  towerOk = 0
  if kind = 4 and laneIdx <= 2 and prevTanks >= 2 then
    gdx = x - enemyX
    gdy = y - enemyY
  ...
  elseif kind = 2 then
    ...
    ' v355: opening brawl mode — until tick 1800 heroes outrank creeps (the leaders aim a third of their early attacks at heroes)
    if worldTick < 1800 and selfHp * 2 >= selfMaxHp then
      score = score + 150
    end if
  elseif kind = 3 then
    score = score + 100
...
if floater = 1 and worldTick >= 1200 and worldTick - lastLaneChange >= 2400 and worldTick mod 600 < 6 then
  ...
  if newLane <> laneIdx and curAllies >= 2 then
    laneIdx = newLane
...
camp = 0
if laneIdx <= 2 and selfLevel >= 12 and prevCampHeroes = 0 and attackRange >= 4 then
  camp = 1
end if
...
if laneIdx <= 2 and canShop() = 1 and owned(21) > 0 and selfPortalCooldown = 0 and forwardDistance < 1000000 and inventorySlot(21) >= 0 and worldTick >= 1800 then
  pdx = selfX - forwardX
  pdy = selfY - forwardY
...
' v352: opening brawl — every hero goes to the map centre with the first waves until tick 1800 (the leaders farm early hero kills there)
if worldTick < 1800 then
  goalX = mapWidth \ 2
  goalY = mapHeight \ 2
end if
