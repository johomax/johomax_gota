' v328 = v325 pushing like richard v153: any lane tower including gates is a target while an allied creep is within 8 tiles, the six-tile tower clamp applies only without creeps, camp from level 7
...
camp = 0
if laneIdx <= 2 and selfLevel >= 7 then
  camp = 1
end if
...
if laneIdx <= 2 and frontD < 1000000 then
  ...
  if eTowerD < 1000000 and prevTanks = 0 then
    tdx = frontX - eTowerX
    tdy = frontY - eTowerY
...
if camp = 1 then
  goalX = enemyX + (mapWidth \ 2 - enemyX) \ 9
  goalY = enemyY + (mapHeight \ 2 - enemyY) \ 9
end if
