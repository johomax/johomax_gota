' v411 = v410 + ranged heroes hold two tiles ahead of the leading allied creep (v396's rule, +58 at n120)
' v410 = v402 re-uploaded for promotion (v385 + carries flee ten seconds from an enemy hero three or more levels above them: +107 n100, +124 n240 vs the v385 content)
...
sub readObject(index)
  ...
  ' v402: carries flee too, but only from a hero three or more levels above them (two levels cost carries 288 in v366)
  if kind = 2 and distance <= 144 and objectLevel(index) >= selfLevel + 3 then
    carryHunterUntil = worldTick + 240
  end if
...
fleeNow = 0
if worldTick < hunterUntil and selfClass <> Ranger and selfClass <> Crossbowman then
  fleeNow = 1
end if
if worldTick < carryHunterUntil and (selfClass = Ranger or selfClass = Crossbowman) then
  fleeNow = 1
end if
if fleeNow = 1 and camp = 0 then
  fleeX = homeX
  fleeY = homeY
...
if laneIdx <= 2 and frontD < 1000000 then
  goalX = frontX
  goalY = frontY
  ' v396: ranged farmers stand two tiles ahead of the leading allied creep (enemy ranged creeps die four tiles behind their melee line)
  if attackRange >= 4 and camp = 0 then
    if enemyX > frontX then
      goalX = frontX + 2
    elseif enemyX < frontX then
      goalX = frontX - 2
    end if
    if enemyY > frontY then
      goalY = frontY + 2
    elseif enemyY < frontY then
      goalY = frontY - 2
    end if
  end if
