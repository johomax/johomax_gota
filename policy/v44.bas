...
' A destroyed objective or lane switch can change the tower after the scan.
if objDead = 1 or laneChanged = 1 then
  objHp = 1000
  i = 0
  while i < n
    if objectKind(i) = 4 and objectId(i) = wpId(oi) then
      objHp = objectHp(i)
    end if
    i = i + 1
  wend
...
holdLine = 0
if routeDead(pushLane * 7 + 3) = 1 and oi >= 4 and oi < 6 and allyCount < 2 and objHp > 400 and worldTick < 4000 then
  holdLine = 1
end if
if holdLine = 1 and prevHold = 0 then
  print "HOLD " ; worldTick
end if
if holdLine = 0 and prevHold = 1 then
  print "HOLD end " ; worldTick
end if
prevHold = holdLine
...
if done = 0 and holdLine = 0 and routing = 0 and towerId <> 0 and lowHp = 0 and nearHeroD2 > 9 then
...
if done = 0 and holdLine = 1 and bestFoot = 0 and farmFoot <> 0 then
  attackTarget(farmFoot)
  done = 1
  act = 16
end if

if done = 0 and holdLine = 1 then
  stepToward(wpX(3), wpY(3), wpX(4), wpY(4), 4)
  holdX = sx
  holdY = sy
  dx = holdX - selfX
  dy = holdY - selfY
...
if done = 0 and holdLine = 0 and routing = 0 and towerId <> 0 and lowHp = 0 then
  attackTarget(towerId)
  done = 1
  act = 6
end if
