...
' v119 = v113 without the sword purchase, so one inventory slot stays free for elixirs (ranged seats ended with 100-360 unspent gold and full slots).
  ...
  if bootsBought = 0 and selfGold >= 100 then
    ...
    if potSlot < 0 and selfGold >= 80 then
      ...
      if axeBought = 0 and selfGold >= 180 then
        ...
        ' v119: no sword — five equipment items leave the sixth inventory slot free for elixirs all game
