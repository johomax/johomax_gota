' v350 = relh-style aggressive mid hero on the v338 base: every seat opens mid and never switches; any tower or guard is a target while an allied creep is within 8 tiles; heroes two levels below us are hunted; dagger-first damage kit (no scroll spam); ranged camp from level 10
...
sub readObject(index)
  ...
  towerOk = 0
  if kind = 4 and laneIdx <= 2 and prevTanks >= 1 then
    towerOk = 1
  end if
...
sub inventory()
  ...
  budget = selfGold
  ' v350: relh's kit order — Crimson Dagger, Ranger Bow, Rune Crossbow, Battle Axe, Knight Armor, boots; a potion; one scroll only with 400 gold spare
  buy(11, 110, 1)
  buy(1, 30, 1)
  buy(14, 150, 1)
  buy(19, 180, 1)
...
camp = 0
if laneIdx <= 2 and selfLevel >= 10 and prevCampHeroes = 0 and attackRange >= 4 then
  camp = 1
end if
