' v409 = v405 (lane 2 start) + roaming to the nearest enemy wave from tick 2400 (v362's rule replacing FLOAT; the fallback goals fixed too)
' v405 = v385 re-uploaded (revert of v403: the closer camp point read +309 at n120 but -171 at n240)
...
sub readObject(index)
  ...
  ' v409: per lane, remember the enemy creep nearest our god (how deep the enemy wave stands on our side)
  if kind = 3 then
    hdx = x - homeX
    hdy = y - homeY
    hd = hdx * hdx + hdy * hdy
    if (x <= 13 or y <= 17) and hd < waveD(0) then
      waveD(0) = hd
      waveX(0) = x
      waveY(0) = y
    end if
    if (x >= 102 or y >= 98) and hd < waveD(2) then
      waveD(2) = hd
      waveX(2) = x
      waveY(2) = y
    end if
    ...
    if laneS <= 14 and hd < waveD(1) then
      waveD(1) = hd
      waveX(1) = x
      waveY(1) = y
    end if
...
if initialized = 0 then
  initialized = 1
  ' v385: everyone starts in lane 2. By class in the same games (v363, 240 heroes): Ranger lane 2 1592 vs lane 0 1242 vs mid 855; Xbow 951/609/475; Arcanist+Lich 677/318/191; melee 311/40/52
  laneIdx = 2
  floater = 1
  lastLaneChange = 0
  switches = 0
...
' v409: khors/relh/richard roam to whichever enemy wave is nearest: every 600 ticks after tick 2400 pick the lane whose enemy wave costs least
' (squared distance of that wave from our god + squared distance from us + 6000 per allied hero in the region; the current lane keeps a 3000 bonus)
if worldTick >= 2400 and worldTick - lastLaneChange >= 1200 and worldTick mod 600 < 6 and camp = 0 then
  bestCost = 1000000
  newLane = laneIdx
  for wl = 0 to 2
    if waveD(wl) < 1000000 then
      wdx = waveX(wl) - selfX
      wdy = waveY(wl) - selfY
      laneCost = waveD(wl) + wdx * wdx + wdy * wdy + 6000 * allyLane0
      if wl = 1 then
        laneCost = laneCost - 6000 * allyLane0 + 6000 * allyLane1
      elseif wl = 2 then
        laneCost = laneCost - 6000 * allyLane0 + 6000 * allyLane2
      end if
      if wl = laneIdx then
        laneCost = laneCost - 3000
      end if
      if laneCost < bestCost then
        bestCost = laneCost
        newLane = wl
      end if
    end if
  ...
  if newLane <> laneIdx then
    ...
    switches = switches + 1
    print "WAVE " ; worldTick ; " lane " ; laneIdx ; " d " ; waveD(0) ; " " ; waveD(1) ; " " ; waveD(2) ; " allies " ; allyLane0 ; " " ; allyLane1 ; " " ; allyLane2
  end if
  waveD(0) = 1000000
  waveD(1) = 1000000
  waveD(2) = 1000000
