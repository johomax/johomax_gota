' v400 = v385 with the non-carry hunter flee lasting twenty seconds instead of ten
...
sub readObject(index)
  ...
  ' v400: flee for twenty seconds instead of ten (a hunter that lost sight of us for ten seconds is often still there)
  if kind = 2 and distance <= 144 and objectLevel(index) >= selfLevel + 2 then
    hunterUntil = worldTick + 480
  end if
