' v398 = v385 with the camp allowed beside one enemy hero once we are level 12
...
camp = 0
' v398: from level 12 one enemy hero near the camp point no longer cancels the camp (the leaders converge on the enemy base late, so a clean camp point is rare)
campGate = 0
if prevCampHeroes = 0 then
  campGate = 1
elseif prevCampHeroes = 1 and selfLevel >= 12 then
  campGate = 1
end if
if laneIdx <= 2 and campGate = 1 and selfDeaths < 4 then
