...
' Top up before reaching the next tower.
potHp = 55
...
while slot < 6
  ...
  slot = slot + 1
wend
if selfHp * 100 < selfMaxHp * potHp then
  if potSlot >= 0 then
    useItem(potSlot)
  else
    if selfGold >= 50 then
      ...
      if potHp = 70 then
        slot = 0
        while slot < 6 and potSlot < 0
          if itemId(slot) = 2 then
            potSlot = slot
            useItem(potSlot)
          end if
          slot = slot + 1
  ...
  if t = selfTeam then
    ...
    if k = 2 then
      if objectAlive(i) = 1 and d2 <= 100 then
        ...
        hp = objectHp(i)
        if d2 <= heroR2 and (lowHp = 0 or d2 * 100 <= rng * rng) then
    ...
    if k = 3 then
      if objectAlive(i) = 1 and d2 <= footR2 and (lowHp = 0 or d2 * 100 <= rng * rng) then
        hp = objectHp(i)
...
' Low-HP heroes return fire in basic range before falling back.
if selfHp * 100 < selfMaxHp * 15 and enemyHeroNear > 0 and bestHero = 0 and bestFoot = 0 then
  walkTo(rx, ry)
  done = 1
  act = 1
end if

if done = 0 and melee = 0 and nearMeleeD2 <= 4 and (lowHp = 0 or (bestHero = 0 and bestFoot = 0)) then
...
if done = 0 and towerId <> 0 and lowHp = 0 then
  ' Let melee allies take tower fire before ranged heroes join the siege.
  tankNear = 0
  if melee = 0 and enemyHeroNear = 0 and towerHp >= 300 then
    i = 0
    while i < n and tankNear = 0
      if objectTeam(i) = selfTeam and objectKind(i) = 2 and objectAlive(i) = 1 then
        c = objectClass(i)
        if c = 0 or c = 4 or c = 5 or c = 9 then
          dx = objectX(i) - towerX
          dy = objectY(i) - towerY
    ...
    wend
    if tankNear = 0 then
      stepToward(towerX, towerY, rx, ry, 6)
      walkTo(sx, sy)
      done = 1
      act = 12
    end if
