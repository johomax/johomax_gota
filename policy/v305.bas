' v305 = v304: farmers step to the lane tower on any power deficit within 8 tiles below 60% health, and drink potions below 70% (deaths were 3-11 per game)
    ...
    if id = 0 then
      ...
      if itemCooldown(itemSlot) = 0 and inOwnSpawn() = 0 then
        ...
        if id = 1 and selfMaxHp - selfHp >= 60 then
          ...
          if laneIdx <= 2 and selfHp * 10 < selfMaxHp * 7 then
            consume = worldTick - hurtTick > tickRate \ 2
          end if
...
if (enemyPower > friendlyPower + 6 and threatDistance < 64) or (laneIdx <= 2 and enemyPower > friendlyPower and threatDistance < 64 and selfHp * 5 < selfMaxHp * 3) then
