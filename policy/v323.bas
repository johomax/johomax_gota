' v323 = v317 + Ranger/Crossbowman prioritise enemy heroes they out-level by two while above two-thirds health
...
sub readObject(index)
  ...
  elseif kind = 2 then
    ...
    ' v323: a healthy ranged carry two levels up takes the 150-XP kill
    if (selfClass = Ranger or selfClass = Crossbowman) and selfLevel >= objectLevel(index) + 2 and selfHp * 3 >= selfMaxHp * 2 then
      score = score + 200
    end if
  elseif kind = 3 then
    score = score + 100
