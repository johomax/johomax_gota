' v387 = v385 with the hunter threshold lowered to one level above us (non-carries flee ten seconds)
...
sub readObject(index)
  ...
  ' v387: one level above us is already a hunter (melee seats still zero 65% of games in lane 2)
  if kind = 2 and distance <= 144 and objectLevel(index) >= selfLevel + 1 then
