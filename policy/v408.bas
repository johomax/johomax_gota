' v408 = v405 + khors-style opening (Crimson Dagger before the scroll) + every ability cast on cooldown
...
sub inventory()
  ...
  if laneIdx <= 2 then
    ' v408: khors-style opening — Crimson Dagger and a potion at spawn (150 gold), scroll on the first return
    buy(11, 110, 1)
    buy(1, 30, 1)
    buy(21, 100, 1)
    buy(8, 100, 1)
...
sub spells()
  for spellSlot = 0 to 3
    ...
    if abilityLevel(spellSlot) > 0 and charges > 0 then
      ' v408: every ability on cooldown when the mana is there (khors casts ~630 times a game, we ~15; regen is 240 mana a minute)
