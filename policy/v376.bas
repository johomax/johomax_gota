' v376 = v363 + move to an empty lane whenever an ally shares ours (every 1200 ticks, max four moves) with the v373 field portal so the move is short
' v373 = v363 + the portal scroll is used from anywhere when the lane tower is at least fifteen tiles nearer the goal than we are (not only from the keep)
...
if floater = 1 and worldTick >= 1200 and worldTick - lastLaneChange >= 1200 and worldTick mod 600 < 6 then
  ...
  ' v376: any ally in our lane region and an empty lane elsewhere is reason enough to move (a lane's 360 XP/min is worth the trip); the field portal makes the trip short
  if newLane <> laneIdx and curAllies >= 1 and bestAllies = 0 and switches < 4 then
    laneIdx = newLane
...
' v373: the scroll works from the field: portal to the lane's most advanced tower whenever that lands us at least fifteen tiles nearer the goal
' (after heals, deaths, switches and long walks; a hosted Crossbowman walked 90 tiles home and back twice with a scroll in its bag)
if laneIdx <= 2 and owned(21) > 0 and selfPortalCooldown = 0 and forwardDistance < 1000000 and inventorySlot(21) >= 0 and threatDistance > 144 then
  pdx = selfX - forwardX
  pdy = selfY - forwardY
  sgx = goalX - selfX
  sgy = goalY - selfY
  tgx = goalX - forwardX
  tgy = goalY - forwardY
  if pdx * pdx + pdy * pdy > 400 and sgx * sgx + sgy * sgy > tgx * tgx + tgy * tgy + 225 then
