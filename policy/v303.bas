' v303 = v302 + farmers prioritise enemy heroes they out-level by two while above two-thirds health (150 XP per kill)
...
sub readObject(index)
  ...
  elseif kind = 2 then
    ...
    ' v303: a healthy farmer two levels up takes the 150-XP kill
    if laneIdx <= 2 and selfLevel >= objectLevel(index) + 2 and selfHp * 3 >= selfMaxHp * 2 then
      score = score + 200
    end if
  elseif kind = 3 then
    score = score + 100
    ...
    if laneIdx <= 2 and objectClass(index) = 1 then
      score = score + 150
    end if
