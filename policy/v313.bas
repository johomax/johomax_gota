' v313 = v312 + enemy heroes we out-level by two get +200 target priority while we are above two-thirds health
...
sub readObject(index)
  ...
  elseif kind = 2 then
    ...
    ' v313: a healthy hero two levels up takes the 150-XP kill (only our own hero's XP counts in mixed rosters)
    if selfLevel >= objectLevel(index) + 2 and selfHp * 3 >= selfMaxHp * 2 then
      score = score + 200
    end if
  elseif kind = 3 then
    score = score + 100
