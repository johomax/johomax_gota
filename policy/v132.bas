...
' v132 = v113 potion economy: keep an elixir stocked whenever gold >= 50, drink at 70% HP (was 55%), no sword so one inventory slot stays free (strong opponents use items ~50x more than we do).
...
while slot < 6
  ...
  slot = slot + 1
wend
' v132: drink at 70% HP (was 55%)
if selfHp * 100 < selfMaxHp * 70 then
  if potSlot >= 0 then
    useItem(potSlot)
  else
    if selfGold >= 50 then
...
if daggerBought = 0 and selfGold >= 110 then
  ...
  if bootsBought = 0 and selfGold >= 100 then
    ...
    if potSlot < 0 and selfGold >= 50 then
      buyItem(2)
    else
      if axeBought = 0 and selfGold >= 180 then
        ...
        ' v132: no sword, so the sixth slot stays free for elixirs
